#pragma once
#define HTML_BUNDLE_SIZE 57668
static std::string html_bundle = 
R"fffc25e9(<!DOCTYPE html>
<html lang="en">
  <head>
    <meta charset="UTF-8" />
...
R"fffc25e9(           onInput=${(e) => setSeed(e.target.value)}
    ...
    function ImgOutput({ width, height, outputStatus, setActiveView }) {
      ...
      return html`
        <div class="framed">
          ...
          ${images.length
            ? html`<div class="buttons gap">
                  <button
                    class="btn-blue"
                    onClick=${() => {
                      setImageAsReference(images[0].src)
                    }}>
                    Use as reference
                  </button>
                  <!--<button>Save</button>-->
                ...
                <p class="faded-text-button">Copy Output Folder</p> `
            : null}
        </div>
      `
    }
    ...
    function Content({
      ...
      return html`<div class="content">
        ...
        <hr />
        ${!modelStatus.model && !modelStatus.justStarted ? html`<progress id="progress-bar" aria-label="Content loading…"></progress>` : ""}
          ...
          ${modelStatus.justStarted
            ? html`<div>
                ...
                <div class="hello-message">
                  ${FILES_DIR.map((dir) => html`<code>${dir}</code>`)}
                </div>
              </div>`
            : html`<div class="io">
    ...
    function mockFetch(url) {
      ...
      if (url.includes("/api/v0/models")) {
        ...
        return new Promise((resolve) =>
          setTimeout(() => resolve({ ok: true, text: () => Promise.resolve("OK") }), 5000)
        )
...
R"fffc25e9(    _generationMockResponse[job] = "404 Not Found"
              }, 5000)
            }, 5000)
    ...
    function updateAverageGenerationTime() {
      ...
      console.log("Average generation time now", window.averageGenerationTime)
    }
    ...
    function App() {
        ...
        for (let i = 0; i < STATUS_CHECK_MAX; i++) {
          ...
          if (code === "SUCCESS") {
            ...
            for (const generation of generations) {
              ...
              output.images.push({ src, generationData: data, seedOffset: seedOffset++ })
            }
            ...
            if (averageGenerationTime) {
              ...
              setOutputStatus({ ok: true, message: "Generating...", progress: "Please wait..." })
            }
          } else {
      ...
      function _verifyGenerationOptions(data) {
        if (!data.prompt) alert("Please enter a prompt to generate an image.")
        if (data.width < 0 || data.height < 0) alert("Width and height must be positive integers.")
        if (data["batch-size"] < 1 || data["batch-count"] < 1) alert("Batch size and count must be at least 1.")
        if (data.seed < -1) alert("Seed must be a positive integer or -1 for random.")
        if (data.steps < 1) alert("Sampling steps must be at least 1.")
        if (data["control-strength"] < 0 || data["control-strength"] > 1) alert("Control strength must be between 0 and 1.")
        if (data["style-ratio"] < 0 || data["style-ratio"] > 100) alert("Style ratio must be between 0 and 100.")
        if (data["cfg-scale"] < 1 || data["cfg-scale"] > 20) alert("CFG scale must be between 1 and 20.")
        if (data["denoising-strength"] < 0 || data["denoising-strength"] > 1) alert("Denoising strength must be between 0 and 1.")
        if (data.mode === "img2img" && !data.image) alert("Please provide reference image.")
      }
